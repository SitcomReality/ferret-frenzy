export class PhaseManager {
  ...
  bind() {
    ...
    this.eventBus.on('race:initiateStart', () => this.setPhase('race'));
    this.eventBus.on('race:started', () => this.setPhase('race'));
    this.eventBus.on('race:finish', ({ race }) => { this._lastRace = race; });
    this.eventBus.on('results:show', () => this.setPhase('results'));
